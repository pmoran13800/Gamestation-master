...
# - no parameters: resync gamestation in Buildroot
# - parameters : the user is running a specific make commande like make sdl2-reconfigure or make clean, so don't sync
# - gamestation-*_defconfig: we MUST apply the patches.SOme of them change the dependencies, they must be done BEFORE we do make the defconfig itself
ifeq ($(MAKECMDGOALS),)
all: | merge buildBR 
else
gamestation-%_defconfig: | merge
