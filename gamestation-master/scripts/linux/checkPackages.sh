...
[[ -z $GAMESTATION_DIR ]] && GAMESTATION_DIR=.
...
# List Gamestation defconfigs
DEFCONFIGS=`ls configs/gamestation-* | cut -d "-" -f 2 | sed s+_defconfig++g`
...
function getGitHubApiUrlFromPackage() {
  package=$1
  makeFile=`[[ -f "$BUILDROOT_DIR/package/$package/$package.mk" ]] && echo "$BUILDROOT_DIR/package/$package/$package.mk" || echo "$GAMESTATION_DIR/package/$package/$package.mk"`
...
function getBR2ConfigVar () {
  package=$1
  configFile=`[[ -f $BUILDROOT_DIR/package/"$package"/Config.in ]] && echo "$BUILDROOT_DIR/package/"$package"/Config.in" || echo "$GAMESTATION_DIR/package/"$package"/Config.in"`
...
function isPackageInArch () {
  ...
  grep -q "^${brvar}=y" "$GAMESTATION_DIR/configs/gamestation-${arch}_defconfig" && echo -en "\tx" || echo -en "\t"
}
...
function updatePackageCommitId () {
  ...
  sed -i "s/\(${brPackageName}_VERSION\s\?=\s\?\)${oldId}/\1${newId}/" "$GAMESTATION_DIR"/package/"${package}"/"${package}".mk
}
...
function comparePiDefconfig() {
  ...
  if [[ $willUpdate == 1 && $skippedDefconfig != *"gamestation-rpi${piVersion}_defconfig"* ]] && \
	  [[ ! -z $selectedPackages && $selectedPackages == *"gamestation-rpi${piVersion}_defconfig"* || -z $selectedPackages ]]
  then
    brKernelHeaders=`grep "BR2_PACKAGE_HOST_LINUX_HEADERS_CUSTOM_" "$BUILDROOT_DIR/configs/raspberrypi${piBRVersion}_defconfig"`
    updateDefconfig "$GAMESTATION_DIR/configs/gamestation-rpi${piVersion}_defconfig" "BR2_PACKAGE_HOST_LINUX_HEADERS_CUSTOM_" "$brKernelHeaders"
    brKernelCommitId=`grep "BR2_LINUX_KERNEL_CUSTOM_REPO_VERSION" "$BUILDROOT_DIR/configs/raspberrypi${piBRVersion}_defconfig"`
    updateDefconfig "$GAMESTATION_DIR/configs/gamestation-rpi${piVersion}_defconfig" "BR2_LINUX_KERNEL_CUSTOM_REPO_VERSION" "$brKernelCommitId"
  fi
  
  #~ Get current kernel branch
  rbxkernelBranch=`getKernelHeaderFromDefconfig "$GAMESTATION_DIR/configs/gamestation-rpi${piVersion}_defconfig"`
  brkernelBranch=`getKernelHeaderFromDefconfig "$BUILDROOT_DIR/configs/raspberrypi${piBRVersion}_defconfig"`
  rbxKernelCommitId=`getKernelCustomRepo "$GAMESTATION_DIR/configs/gamestation-rpi${piVersion}_defconfig"`
  ...
  header="\e[1mPackage\t   Gamestation version\t   Buildroot version\t   Update ?\e[0m"
  echo -e "$header
...
function comparePcDefconfig() {
  ...
  #~ if [[ $willUpdate == 1 && $skippedDefconfig != *"gamestation-rpi${piVersion}_defconfig"* ]] ; then
  #~ fi
  rbxkernelBranch=`getKernelHeaderFromDefconfig "$GAMESTATION_DIR/configs/gamestation-${x86Arch}_defconfig"`
  brkernelBranch=`getKernelHeaderFromDefconfig "$BUILDROOT_DIR/configs/pc_x86_64_efi_defconfig"`
  rbxKernelCustomValue=`getKernelCustomValue "$GAMESTATION_DIR/configs/gamestation-${x86Arch}_defconfig"`
  ...
  header="\e[1mPackage\t   Gamestation version\t   Buildroot version\t   Update ?\t   Max\e[0m"
  echo -e "$header
Kernel branch\t${rbxkernelBranch}\t${brkernelBranch}\t${updateKernelBranch}
...
function mainEmulators () {
  ...
  header="\e[1mEmulator / Package\t   Gamestation version\t   Most recent version\t   Update ?\t   `echo $DEFCONFIGS | sed "s+ +\t   +g"`\e[0m\n"
