...
recallog "---- gamestation-config.sh ----"
...
if [ "$command" == "audio" ];then
    ...
    rm /gamestation/share/system/.asoundrc 2>/dev/null
...
rb_wifi_configure() {
	[ "$1" = "1" ] && X="" || X="$1"
	settings_ssid=`egrep "^wifi${X}.ssid" /gamestation/share/system/gamestation.conf|sed "s/wifi${X}.ssid=//g"`
	settings_key=`egrep "^wifi${X}.key" /gamestation/share/system/gamestation.conf|sed "s/wifi${X}.key=//g"`
...
if [[ "$command" == "wifi" ]]; then
	...
	#Configure with ES settings
        ...
        cat > "/var/lib/connman/gamestation.config" <<EOF
[global]
Name=gamestation
...
EOF
        if test "${psk}" != ""
        then
        echo "Passphrase=${psk}" >> "/var/lib/connman/gamestation.config"
        fi
	# Configure with gamestation.conf settings
	for i in {1..3}; do
		rb_wifi_configure $i&
	done
...
if [[ "$command" == "hcitoolscan" ]]; then
    ...
    /gamestation/scripts/bluetooth/"$scanningDaemon" & ( PID=$! ; sleep 15 ; kill -15 $PID)
    PYTHONIOENCODING=UTF-8 /gamestation/scripts/bluetooth/test-device list
    exit 0
fi
...
if [[ "$command" == "hiddpair" ]]; then
    ...
    /gamestation/scripts/bluetooth/simple-agent -c NoInputNoOutput -i hci0 "$mac" 2>&1 | recallog
    connected=${PIPESTATUS[0]}
    if [ $connected -eq 0 ]; then
        hcitool con | grep $mac1
        if [[ $? == "0" ]]; then
            recallog "bluetooth : $mac1 connected !"
            /gamestation/scripts/bluetooth/test-device trusted "$mac" yes
...
if [[ "$command" == "forgetBT" ]]; then
    for mac in $(find /var/lib/bluetooth/ -type d -maxdepth 2 -mindepth 2 | grep -v "/cache$" | cut -d "/" -f 6) ; do 
        ...
        /gamestation/scripts/bluetooth/test-device remove "$mac"
    done
    exit 0
fi

if [[ "$command" == "updateesinput" ]]; then
    inputSrcFile=/gamestation/share_init/system/.emulationstation/es_input.cfg
    inputDstFile=/gamestation/share/system/.emulationstation/es_input.cfg
    ...
    sed -e "/inputConfig type=\"joystick\" deviceName=\"${deviceName}\" deviceGUID=\"${deviceGUID}\"/,/<\/inputConfig>/d" \
        ...
        "${inputDstFile}" | xmllint --format -dtdvalid /gamestation/scripts/es_input.dtd - > $tmpFile
...
recallog -e "gamestation-config.sh: unknown command $command"
