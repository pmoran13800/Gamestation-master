#!/bin/bash
gamestationupdateurl="http://archive.gamestation.com/updates/v1.0"
systemsetting="python /usr/lib/python2.7/site-packages/configgen/settings/gamestationSettings.pyc"
gamestationFiles="boot.tar.xz root.tar.xz boot.tar.xz.sha1 root.tar.xz.sha1 root.list"

arch=$(cat /gamestation/gamestation.arch)
updatetype="`$systemsetting  -command load -key updates.type`"
upgradeDir="/gamestation/share/system/upgrade"

function calcDownloadedSize() {
    size=0
    for dlFile in ${gamestationFiles} ; do
...
size="0"
for file in ${gamestationFiles}; do
  url="${gamestationupdateurl}/${updatetype}/${arch}/${file}"
...
freespace=`df -k /gamestation/share | tail -1 | awk '{print $4}'`
if [ $? -ne 0 ];then
  recallog -e "Unable to get freespace for /gamestation/share"
  exit 5
fi
diff=$((freespace - size))
if [[ "$diff" -lt "0" ]]; then
  recallog -e "Not enough space on /gamestation/share to download the update"
  exit 6
fi
