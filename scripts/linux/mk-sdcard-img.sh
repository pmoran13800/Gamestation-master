...
then
    ...
    ScriptDir=$(dirname ${0})
    echo "Hint: -ad ${ScriptDir}/../../output/images/gamestation" >&2
    echo >&2
    print_usage
    exit 1
fi
...
then
    ...
    ScriptDir=$(dirname ${0})
    echo "Hint: -o ${ScriptDir}/../../output/sdimg/gamestation.img" >&2
    echo >&2
    print_usage
    exit 1
fi
...
SDCARD_PWD="/tmp/gamestation_mksdcard${$}"
...
then
    if mkfs.ext4 -L "GAMESTATION" "${SDCARD_P2_FILE_PATH}" > /dev/null
    then
