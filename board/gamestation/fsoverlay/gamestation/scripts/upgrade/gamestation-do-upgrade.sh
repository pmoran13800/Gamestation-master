...
ADDITIONAL_PARAMETERS="?source=gamestation"
...
echoerr "------------ Will process to the gamestation upgrade from ${UPGRADE_URL} ------------"
...
for file in ${FILES_TO_UPGRADE}; do
  FILE_URL="${UPGRADE_URL}/${GAMESTATION_URL}/${ARCH}/${file}"
...
/gamestation/scripts/upgrade/gamestation-do-prereboot-upgrade.sh --upgrade-dir "$UPGRADE_DIR"
if [ $? -ne 0 ];then
