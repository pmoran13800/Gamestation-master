...
rs_sync() {
    ...
    if rsync $RSYNCOPT -v --modify-window=2 --delete-during "/gamestation/share/" "${MOUNTDIR}" # modify-window because all file system such as fat32 doesn't have the same time precision
    then
	EXITCODE=0
    fi
