...
mlRomDir=/gamestation/share/roms/moonlight
...
function mlPair {
  ...
  if [ -z "$1" ]; then
    ...
    cp /gamestation/share_init/system/configs/moonlight/moonlight.conf $mlConf
...
case $1 in
  ...
  clean)
    shift
    mlClean "$1" "fullmode"
    echo 'You can now pair again your gamestation with a PC'
    ;;
  *)
