################################################################################
#
# gamestation-romfs-supergrafx
#
################################################################################

# Package generated with :
# ./scripts/linux/empack.py --system supergrafx --extension '.pce .PCE .sgx .SGX .zip .ZIP' --fullname 'Supergrafx' --platform supergrafx --theme supergrafx libretro:mednafen_supergrafx:BR2_PACKAGE_LIBRETRO_BEETLE_SUPERGRAFX libretro:pce:BR2_PACKAGE_LIBRETRO_BEETLE_PCE

# Name the 3 vars as the package requires
GAMESTATION_ROMFS_SUPERGRAFX_SOURCE = 
GAMESTATION_ROMFS_SUPERGRAFX_SITE = 
GAMESTATION_ROMFS_SUPERGRAFX_INSTALL_STAGING = NO
# Set the system name
SYSTEM_NAME_SUPERGRAFX = supergrafx
SYSTEM_XML_SUPERGRAFX = $(@D)/$(SYSTEM_NAME_SUPERGRAFX).xml
# System rom path
SOURCE_ROMDIR_SUPERGRAFX = $(GAMESTATION_ROMFS_SUPERGRAFX_PKGDIR)/roms

# CONFIGGEN_STD_CMD is defined in gamestation-romfs, so take good care that
# variables are global across buildroot


ifneq ($(BR2_PACKAGE_LIBRETRO_BEETLE_SUPERGRAFX)$(BR2_PACKAGE_LIBRETRO_BEETLE_PCE),)
define CONFIGURE_MAIN_SUPERGRAFX_START
	$(call GAMESTATION_ROMFS_CALL_ADD_SYSTEM,$(SYSTEM_XML_SUPERGRAFX),Supergrafx,$(SYSTEM_NAME_SUPERGRAFX),.pce .PCE .sgx .SGX .zip .ZIP,supergrafx,supergrafx)
endef

ifneq ($(BR2_PACKAGE_LIBRETRO_BEETLE_SUPERGRAFX)$(BR2_PACKAGE_LIBRETRO_BEETLE_PCE),)
define CONFIGURE_SUPERGRAFX_LIBRETRO_START
	$(call GAMESTATION_ROMFS_CALL_START_EMULATOR,$(SYSTEM_XML_SUPERGRAFX),libretro)
endef
ifeq ($(BR2_PACKAGE_LIBRETRO_BEETLE_SUPERGRAFX),y)
define CONFIGURE_SUPERGRAFX_LIBRETRO_MEDNAFEN_SUPERGRAFX_DEF
	$(call GAMESTATION_ROMFS_CALL_ADD_CORE,$(SYSTEM_XML_SUPERGRAFX),mednafen_supergrafx)
endef
endif

ifeq ($(BR2_PACKAGE_LIBRETRO_BEETLE_PCE),y)
define CONFIGURE_SUPERGRAFX_LIBRETRO_PCE_DEF
	$(call GAMESTATION_ROMFS_CALL_ADD_CORE,$(SYSTEM_XML_SUPERGRAFX),pce)
endef
endif

define CONFIGURE_SUPERGRAFX_LIBRETRO_END
	$(call GAMESTATION_ROMFS_CALL_END_EMULATOR,$(SYSTEM_XML_SUPERGRAFX))
endef
endif



define CONFIGURE_MAIN_SUPERGRAFX_END
	$(call GAMESTATION_ROMFS_CALL_END_SYSTEM,$(SYSTEM_XML_SUPERGRAFX),$(SOURCE_ROMDIR_SUPERGRAFX),$(@D))
endef
endif

define GAMESTATION_ROMFS_SUPERGRAFX_CONFIGURE_CMDS
	$(CONFIGURE_MAIN_SUPERGRAFX_START)
	$(CONFIGURE_SUPERGRAFX_LIBRETRO_START)
	$(CONFIGURE_SUPERGRAFX_LIBRETRO_MEDNAFEN_SUPERGRAFX_DEF)
	$(CONFIGURE_SUPERGRAFX_LIBRETRO_PCE_DEF)
	$(CONFIGURE_SUPERGRAFX_LIBRETRO_END)
	$(CONFIGURE_MAIN_SUPERGRAFX_END)
endef

$(eval $(generic-package))
